...
const MetricsSnapshot = mongoose.models.MetricsSnapshot || mongoose.model('MetricsSnapshot', metricsSnapshotSchema);
...
function pruneOldEvents() {
  ...
  while (eventBuffer.length > 0 && new Date(eventBuffer[0].timestamp).getTime() < cutoff) {
    eventBuffer.shift();
  }
}
...
function computeMetrics() {
  ...
  const rateLimitHits = eventBuffer.filter(e => e.type === 'RATE_LIMIT_EXCEEDED').length;
  ...
  return {
    ...
    requestsPerSecond: parseFloat((total / (WINDOW_MS / 1000)).toFixed(3)),
...
// REST endpoints
app.get('/health', (req, res) => res.json({ status: 'ok', service: 'analytics', timestamp: new Date().toISOString() }));
...
app.get('/logs', async (req, res) => {
  try {
    const { page = 1, limit = 50, type, ip, userId, startDate, endDate } = req.query;
    const filter = {};
    ...
    const [logs, total] = await Promise.all([
      RequestLog.find(filter).sort({ timestamp: -1 }).skip(skip).limit(parseInt(limit)).lean(),
...
app.get('/logs/abuse', async (req, res) => {
  try {
    ...
    const logs = await RequestLog.find({ type: 'RATE_LIMIT_EXCEEDED' })
      .sort({ timestamp: -1 })
...
async function processQueue() {
  ...
  try {
    ...
    const events = results.filter(Boolean).map(item => {
    ...
    if (events.length > 0) {
      ...
      try {
        await RequestLog.insertMany(events, { ordered: false });
        logger.debug(`[Worker] Persisted ${events.length} events to MongoDB`);
      } catch (err) {
        // Ignore duplicate key errors from upsert
        if (err.code !== 11000) {
          logger.error('[Worker] MongoDB insert error:', err.message);
        }
      }

      // Emit live update to all connected Socket.IO clients
      const metrics = computeMetrics();
      io.emit('metrics:update', metrics);
      io.emit('logs:new', events.slice(-10)); // Last 10 events
    }
  } catch (err) {
    if (!isShuttingDown) {
...
// Periodic metrics snapshot to MongoDB
async function saveMetricsSnapshot() {
  try {
    ...
    logger.error('[Worker] Snapshot error:', err.message);
  }
}
...
async function bootstrap() {
  // Connect MongoDB
  await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/apishield');
  logger.info('✅ MongoDB connected');

  // Connect Redis consumer
  redisConsumer = createClient({ url: process.env.REDIS_URL || 'redis://localhost:6379' });
  redisConsumer.on('error', err => logger.error('❌ Redis error:', err.message));
  await redisConsumer.connect();
  logger.info('✅ Redis consumer connected');

  // Start polling loop
  const pollLoop = setInterval(processQueue, POLL_INTERVAL_MS);
  const snapshotLoop = setInterval(saveMetricsSnapshot, SNAPSHOT_INTERVAL_MS);

  // Start HTTP server
  const PORT = process.env.PORT || 3001;
  httpServer.listen(PORT, '0.0.0.0', () => {
    logger.info(`🚀 Analytics Service running on port ${PORT}`);
    logger.info(`📡 Socket.IO ready for real-time streaming`);
  });
